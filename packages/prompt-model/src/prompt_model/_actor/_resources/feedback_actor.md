...
## `<prompt>` — the prompt being optimized, as a JSON tree
...
Each node has a `node_type` and (for everything except the root) an `id`. Container nodes carry a `children` list; leaves carry their content in `text`. A small fragment looks like:
      ...
      "children": [
        {
          "node_type": "Paragraph",
          "id": "1.1",
          "text": "Read a short story and answer a causal-judgement question.",
          "examples": {
            "children": [
              { "id": "1.1.e1", "text": "Did X cause Y? -> Yes" }
            ]
          },
          "guidance": null
        }
...
Annotation groups (`examples`, `guidance`) attach **only** to `Paragraph` or `ListItem`. Groups themselves have no `id` and cannot be targeted directly — only their `Annotation` children can. A host may carry at most one `examples` group and at most one `guidance` group.
...
- `host_id` must be the id of a `Paragraph` or `ListItem` that **literally appears in the `<prompt>` JSON above**. Verify both before emitting.
- It must **not** be the id of a `Section`, `List`, `Document`, `CodeBlock`, `Blockquote`, or `Table`. Those node types cannot host annotations and the action will be skipped.
...
- Every `id`, `host_id`, and `target` you emit must appear verbatim in the `<prompt>` JSON you received. Do not invent IDs.
