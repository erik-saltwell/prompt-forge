...
def _result(*signals: IssueSignal, preserve: list[str] | None = None) -> MetricResult:
    return MetricResult(
        score=0.5,
        assessment="ok",
...
def test_single_signal_produces_one_bucket() -> None:
    result: AggregationResult = aggregate([("tone", _result(_signal()))])
...
def test_identical_signals_dedupe_with_count() -> None:
    ...
    result: AggregationResult = aggregate([("tone", _result(s1, s2))])
    assert len(result.buckets) == 1
...
def test_document_sentinel_routes_to_its_own_bucket() -> None:
    ...
    result: AggregationResult = aggregate([("tone", _result(s_doc, s_node))])
...
def test_different_metric_names_do_not_dedupe() -> None:
    s1: IssueSignal = _signal()
    s2: IssueSignal = _signal()
    result: AggregationResult = aggregate([("tone", _result(s1)), ("accuracy", _result(s2))])
    assert len(result.buckets[0].signals) == 2
...
def test_base_signal_prefers_one_with_suggested_prompt_change() -> None:
    ...
    result: AggregationResult = aggregate([("tone", _result(s_no_suggestion, s_with_suggestion))])
...
def test_top_k_cap_keeps_highest_counts() -> None:
    ...
    inputs: list[tuple[str, MetricResult]] = [("tone", _result(s)) for s in distinct_signals]
    extra_dupes: list[tuple[str, MetricResult]] = [("tone", _result(_signal(success_criterion="criterion 0"))) for _ in range(5)]
...
def test_preserve_is_global_deduped_union_in_first_seen_order() -> None:
    ...
    r2: MetricResult = _result(_signal(culprit_node_id="3.4"), preserve=["keep examples", "keep structure"])
    result: AggregationResult = aggregate([("tone", r1), ("accuracy", r2)])
...
def test_results_with_no_signals_skip_buckets_but_keep_preserve() -> None:
    ...
    result: AggregationResult = aggregate([("tone", clean)])
    assert result.buckets == []
